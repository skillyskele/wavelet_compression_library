...
double data[NUM_CHANNELS][SIGNAL_LENGTH]; // 2D array representation....should be NUM_SAMPLES later


int countDecimalPlaces(double value)
{
    int decimalPlaces = 0;
    double fractionalPart = value - floor(value); // Get fractional part only

    while (fractionalPart > 0 && decimalPlaces <= 16)
    {                                            
        fractionalPart *= 10.0;                  // Scale by 10
        fractionalPart -= floor(fractionalPart); // Remove integer part
        decimalPlaces++;
    }
...
// the data will have dimensions [l, t]
double find_tolerance(double nmse){
    double tolerance;
    if (nmse == 0) {
        tolerance = 1;
    } else {
        tolerance = 0.5 * pow(10, -(countDecimalPlaces(nmse)+1)); // if nmse is 1.023, then it takes returns .0005 since there are 3 places past the decimal point, and it needs to be one past that
    }
    return tolerance;
}

// take mean per row
// subtract that mean from each element in that row
void mean_center_rows(double* data, int num_rows, int num_cols) {
    for (int i = 0; i < num_rows; i++) {
        double sum = 0.0;
        for (int j = 0; j < num_cols; j++) {
            sum += data[i * num_cols + j];
        }
        double mean = sum / num_cols;
        for (int j = 0; j < num_cols; j++) {
            data[i * num_cols + j] -= mean;
        }
    }
}
...
// Main function
// could take in inputs double cr, 
void main()
{
